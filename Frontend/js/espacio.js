...
function populateTable(data) {
  ...
  data.forEach((espacio) => {
    ...
    const botonTexto = espacio.estado === 1 ? 'Deshabilitar' : 'Activar';
    ...
    const row = `
            ...
            <tr>
                ...
                <td>
                    <a href="espaciosIndividualAdmin.html" class="btn btn-primary btn-sm btn-ver" data-id="${espacio.id_espacio}">Ver</a>
                    <a href="updateEspacio.html" class="btn btn-primary btn-sm btn-ver" data-id="${espacio.id_espacio}">Editar</a>
                    <a href="#" class="btn btn-primary btn-sm btn-delete" data-id="${espacio.id_espacio}" data-estado="${espacio.estado}">${botonTexto}</a>
                </td>
            </tr>
        `;
    tableBody.innerHTML += row;
  ...
  botonesEliminar.forEach((boton) => {
    boton.addEventListener("click", function (event) {
      event.preventDefault();
      const idEspacio = event.target.getAttribute("data-id");
      const estadoActual = parseInt(event.target.getAttribute("data-estado"));
      const nuevoEstado = estadoActual === 1 ? 2 : 1;
      if (
        confirm(
          `¿Estás seguro de que deseas ${nuevoEstado == 1 ? "activar" : "desactivar"} este espacio id: ` +
            idEspacio +
            "?"
        )
      ) {
...
function eliminarEspacio(idEspacio, nuevoEstado){
  ...
  fetch(
    "http://localhost/ProyectoAnalisis1/Backend/index.php/espacios/deleteIdEspacio",
    {
      ...
      body: JSON.stringify({ id_condo: idCondo, id_espacio: idEspacio, nuevo_estado: nuevoEstado }),
    }
  )
    .then((response) => {
      if (!response.ok) {
